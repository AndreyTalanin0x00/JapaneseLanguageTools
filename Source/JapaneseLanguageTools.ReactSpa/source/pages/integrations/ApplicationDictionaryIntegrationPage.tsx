...
import { downloadExportBlob, exportApplicationDictionary, getExportBlobMetadata } from "@/api/applicationDictionaryMethods.export";
import { importApplicationDictionary, uploadImportBlob } from "@/api/applicationDictionaryMethods.import";
...
import styles from "./ApplicationDictionaryIntegrationPage.module.css";

const { Title } = Typography;

const ApplicationDictionaryIntegrationPage = () => {
  ...
  const [exportConfigurationForm] = Form.useForm<ApplicationDictionaryExportConfiguration>();

  const initialExportConfiguration: ApplicationDictionaryExportConfiguration = useMemo(
    () => ({
  ...
  const onExportFormFinish = useCallback((applicationDictionaryExportConfiguration: ApplicationDictionaryExportConfiguration) => {
    const { snapshotType, snapshotFileFormat } = applicationDictionaryExportConfiguration;

    const exportApplicationDictionaryRequestModel: ExportApplicationDictionaryRequestModel = { snapshotType, snapshotFileFormat };

    setExportLoading(true);
    exportApplicationDictionary(exportApplicationDictionaryRequestModel)
      .then((exportApplicationDictionaryResponseModel) => {
        const { blobReference } = exportApplicationDictionaryResponseModel;
        ...
        >
          <Space style={{ display: "flex" }} direction="vertical">
            <Form form={exportConfigurationForm} initialValues={initialExportConfiguration} onFinish={onExportFormFinish} onFinishFailed={onExportFormFinishFailure} size="small" labelCol={{ span: 8 }} wrapperCol={{ span: 16 }}>
              <Form.Item name={keyOf<ApplicationDictionaryExportConfiguration>("snapshotType")} label="Snapshot Type">
              ...
              <Form.Item name={keyOf<ApplicationDictionaryExportConfiguration>("snapshotFileFormat")} label="Snapshot File Format">
  ...
  interface ApplicationDictionaryImportConfiguration {
    uploadFiles: UploadFile[];
  }

  const [importConfigurationForm] = Form.useForm<ApplicationDictionaryImportConfiguration>();

  const initialImportConfiguration: ApplicationDictionaryImportConfiguration = useMemo(
    () => ({
      uploadFiles: [],
    }),
    []
  ...
  const onImportFormFinish = useCallback((applicationDictionaryImportConfiguration: ApplicationDictionaryImportConfiguration) => {
    const { uploadFiles } = applicationDictionaryImportConfiguration;
    ...
    const importApplicationDictionaryRequestModel: ImportApplicationDictionaryRequestModel = {
      blobReferences,
    };

    setImportLoading(true);
    importApplicationDictionary(importApplicationDictionaryRequestModel)
      .then(() => {
        ...
        >
          <Space style={{ display: "flex" }} direction="vertical">
            <Form form={importConfigurationForm} initialValues={initialImportConfiguration} onFinish={onImportFormFinish} onFinishFailed={onImportFormFinishFailure} size="small" labelCol={{ span: 8 }} wrapperCol={{ span: 16 }}>
              <Form.Item name={keyOf<ApplicationDictionaryImportConfiguration>("uploadFiles")} label="Upload Files" valuePropName="fileList" getValueFromEvent={getUploadValueFromEvent} noStyle>
                <Upload.Dragger
                  multiple={true}
                ...
                >
                  ...
                  <p className="ant-upload-hint">Accepted file extensions: {FileConstants.applicationDictionaryImportUploadAccept.split(",").join(", ")}. Please, note that files will be processed in the order they are uploaded.</p>
                </Upload.Dragger>
              </Form.Item>
            </Form>
            {mobileBrowserMode ? importControlsJsx : undefined}
  ...
  return (
    <>
      <Title level={4}>Application Dictionary Integrations</Title>
      <Tabs
...
export default ApplicationDictionaryIntegrationPage;
